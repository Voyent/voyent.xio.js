...
(function (b) {
    ...
    function getStoredCloudPushURI() {
        var cloudURI = voyent.getLocalStorageItem(XIO_CLOUD_URI_KEY);
        if (cloudURI) {
            return cloudURI;
        }

        var notifyBack = voyent.getCloudPushId();
        if (notifyBack) {
    ...
    function setStoredCloudPushURI(cloudPushURI) {
        voyent.setLocalStorageItem(XIO_CLOUD_URI_KEY, cloudPushURI);
    }

    function removeStoredCloudPushURI(cloudPushURI) {
        voyent.removeLocalStorageItem(XIO_CLOUD_URI_KEY);
    }

    function getStoredSmsURI() {
        var smsURI = voyent.getLocalStorageItem(XIO_SMS_URI_KEY);
        return smsURI || null;
    }

    function setStoredSmsURI(smsURI) {
        voyent.setLocalStorageItem(XIO_SMS_URI_KEY, smsURI);
    }

    function removeStoredSmsURI() {
        voyent.removeLocalStorageItem(XIO_SMS_URI_KEY);
    }

    function getStoredEmailURI() {
        var emailURI = voyent.getLocalStorageItem(XIO_EMAIL_URI_KEY);
        return emailURI || null;
    }

    function setStoredEmailURI(emailURI) {
        voyent.setLocalStorageItem(XIO_EMAIL_URI_KEY, emailURI);
    }

    function removeStoredEmailURI() {
        voyent.removeLocalStorageItem(XIO_EMAIL_URI_KEY);
    }
    ...
    function getCloudPushURIHash(cloudPushURI) {
        ...
        return b.io.auth.getLastKnownUsername() + '_' + getDatabaseId(theURI);
    }
    ...
    function addCloudConfiguration(host, account, realm, username, theURI, transport, setConfig) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            return b.$.post(pushServiceURL, config)
    ...
    xio.push = {
        ...
        registerCloudPush: function (host, account, realm, username) {
            ...
            return new Promise(function (resolve, reject) {
                ...
                voyent.setLocalStorageItem('xio.host', host);
                voyent.setLocalStorageItem('xio.account', account);
                voyent.setLocalStorageItem('xio.realm', realm);
                voyent.setLocalStorageItem('xio.username', username);

                //intercept launchFailed to report a reject, then set it back
                var origLaunchFailed = voyent.launchFailed;
                voyent.launchFailed = function(){
                    console.log('launchFailed from registerCloudPush()');
                    reject('could not launch the voyent client app');
                    voyent.launchFailed = origLaunchFailed;
                };
                window.xioCloudRegistrationCallback = function(results) {
                    ...
                    voyent.xio.push.registerCloudPush(
                        voyent.getLocalStorageItem('xio.host'),
                        voyent.getLocalStorageItem('xio.account'),
                        voyent.getLocalStorageItem('xio.realm'),
                        voyent.getLocalStorageItem('xio.username')
                    );
                    voyent.launchFailed = origLaunchFailed;
                    resolve();
                };
