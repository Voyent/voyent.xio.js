...
(function (b) {
    ...
    function getStoredSmsURI() {
        var smsURI = bridgeit.getLocalStorageItem(XIO_SMS_URI_KEY);
        return smsURI || null;
    }

    function setStoredSmsURI(smsURI) {
        bridgeit.setLocalStorageItem(XIO_SMS_URI_KEY, smsURI);
    }

    function removeStoredSmsURI() {
        bridgeit.removeLocalStorageItem(XIO_SMS_URI_KEY);
    }

    function getStoredEmailURI() {
        var emailURI = bridgeit.getLocalStorageItem(XIO_EMAIL_URI_KEY);
        return emailURI || null;
    }

    function setStoredEmailURI(emailURI) {
        bridgeit.setLocalStorageItem(XIO_EMAIL_URI_KEY, emailURI);
    }

    function removeStoredEmailURI() {
        bridgeit.removeLocalStorageItem(XIO_EMAIL_URI_KEY);
    }
    ...
    function addCloudConfiguration(host, account, realm, username, theURI, transport) {
    ...
    function getNotificationHandler(caller){
        console.log('creating notification handler', caller, new Date().toISOString());
    ...
    xio.push = {
        ...
        join: function (group) {
            ...
            this.client.on(getNamespaceGroupNotifyEvent(group), getNotificationHandler('joining ' + group));
        ...
        rejoinGroups: function () {
            ...
            for (var i = 0; i < this.groups.length; i++) {
                ...
                var notificationHandler =  getNotificationHandler('removing and rejoining '+ group);
                this.client.removeListener(getNamespaceGroupNotifyEvent(group),notificationHandler);
                this.client.on(ns, notificationHandler);
                this.client.emit('join', group);
            }
        },
