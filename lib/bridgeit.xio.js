    ...
    function sendGroupPush(host, account, realm, group, message) {
        return new Promise(
            function (resolve, reject) {
                ...
                //Basic check of what the message is and properly turn it into an JSON object.
                var messageObject = message;
                if (isString(message)) {
                    messageObject = {message: message};
                }

                console.log('preparing to send push notification', group, JSON.stringify(messageObject));

                b.$.post(groupPushURL, messageObject, null, false, 'application/json')
                    .then(function (response) {
